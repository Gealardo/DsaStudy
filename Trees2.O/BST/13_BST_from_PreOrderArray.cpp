...
Node* BstByPreorder(vector<int>&preorder,int mini,int maxi,int &i)
{

   // base case i cross no.of ele in preO, ele out of range.

   if(i>=preorder.size()) return nullptr;

   if(preorder[i]<mini || preorder[i]>maxi) return nullptr;

   Node* rootnode=new Node(preorder[i++]);
 
   rootnode->left=BstByPreorder(preorder,mini,rootnode->data,i);

   rootnode->right=BstByPreorder(preorder,rootnode->data,maxi,i);
...
int main(){
 ...
 int maxi=INT_MAX;
 int i=0;
 Node* rootnode=BstByPreorder(preorder,mini,maxi,i);
 vector<vector<int>>BSTfromPre=LevelOrder(rootnode);
 cout<<"BST by PreOrder: ";
 printLO(BSTfromPre);
